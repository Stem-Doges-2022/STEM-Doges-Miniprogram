...
Page({
  ...
  getRanking() {
    ...
    let people = [];
    db.collection('userInfo').where({isFinished:true, lastWeekSeen: app.weekNum}).count({
      success: function(res) {
        console.log(res);
        for (let i=0 ; i<res.total; i+=20){
          db.collection('userInfo').where({isFinished:true, lastWeekSeen: app.weekNum}).skip(i).get({
            success: function(res) {
